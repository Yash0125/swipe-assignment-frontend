...
const InvoiceEditableLine = ({ invoice = {}, setinvoices, invoices }) => {
  ...
  const handleCalculateTotal = () => {
    const updatedInvoices = invoices?.map((obj) => {
      if (obj?.id === invoice?.id) {
        ...
        const taxAmount = parseFloat(subTotal * (obj?.taxRate / 100)).toFixed(
          2
        );
  ...
  return (
    <>
      <tr>
        <td style={{ textAlign: "center" }}>{invoice?.invoiceNumber}</td>
        <td>
          <Form.Control
            type="date"
...
export default InvoiceEditableLine;
